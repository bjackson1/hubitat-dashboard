definition(
    name: "Central Heating Controller Parent - DEV",
    namespace: "brett",
...
def selectDevices() {
    ...
    return dynamicPage(name: "mainPage", title: "Setup Room", install: true, uninstall: true) {
		section {
			input "zoneName", "text", title: "Name this Zone", submitOnChange: false
            input("newZone", "button", title: "New Zone", submitOnChange: false)
...
def appButtonHandler(buttonName) {
    if(buttonName == "newZone") {
        addChildApp("brett", "Central Heating Controller Child - DEV", settings["zoneName"])
    }
}
...
def renderDashboard() {
    zonesJs = "window.heatingZones=["
    apps = getChildApps()
    apps.each {
        zonesJs += "{id: '${it.getId()}', label: '${it.getLabel()}'},"
    }
    zonesJs += "]"
    log.debug(zonesJs)
    data = "<html><head><script>${zonesJs}</script><script defer=\"defer\" src=\"${getFullLocalApiServerUrl()}/static/dashboard.js?access_token=${state.accessToken}\"></script></head><body><div id=\"root\"></div></body></html>";
